...
export default function DuneChart({ queryId, maxRows = 100 }){
  ...
  if (loading) return (
    <div className="bg-card p-6 rounded-md h-[500px] flex flex-col items-center justify-center overflow-auto border border-border dark:border-0">
      <div className="w-8 h-8 border-4 border-muted-foreground/30 border-t-transparent rounded-full animate-spin mb-3" />
      <div className="text-sm text-muted-foreground">Loading Dune data...</div>
    </div>
  )
    ...
    return (
      <div className="bg-card p-4 rounded-md h-[500px] flex flex-col border border-border dark:border-0">
        <div className="flex items-center justify-between mb-3">
          ...
          <div className="text-xs text-muted-foreground">Showing {showingFrom}-{showingTo} of {total}</div>
        </div>
              ...
              <thead className="bg-muted">
                <tr>
                  {keys.map(k => (
                    <th
                      key={k}
                      className="text-center pr-4 py-2 font-semibold sticky top-0 z-10 bg-muted"
                    >
                      {k}
                    </th>
                  ))}
                  ...
                  <tr key={i} className="border-b border-border">
                    {keys.map(k => {
                      ...
                      if (String(k).toLowerCase().includes('rank')) {
                        return <td key={k} className="pr-4 py-2 align-top text-sm text-center">{s}</td>
                      }
                      const cleaned = s.replace(/[^0-9.\-]/g, '')
                      const num = cleaned === '' ? NaN : Number(cleaned)
                      const cell = !isNaN(num) ? `$${Math.trunc(num).toLocaleString()}` : s
                      return <td key={k} className="pr-4 py-2 align-top text-sm text-center">{cell}</td>
                    })}
                  </tr>
                ))}
              </tbody>
