...
export default function PriceInfo({tokenIn,tokenOut,amountIn, amountOut ,reserves, priceImpact, slippage, minAmountOut }:IPriceInfo){
  ...
  return(
    <div className="mb-4 space-y-2 mt-4">
        <div className="p-3 bg-blue-50 dark:bg-muted rounded-lg space-y-2">
        ...
        <div className="flex justify-between text-sm">
          {/* 滑点数 */}
          <span className="text-muted-foreground">{t('Slippage Tolerance')}</span>
          <span className="font-semibold">{slippage}%</span>
        </div>
        <div className="flex justify-between text-sm">
          {/* 最少收到: amountOut - (1 - slippage / 100) */}
          <span className="text-muted-foreground">{t('Minimum Received')}</span>
